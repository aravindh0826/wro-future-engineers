...
# ── Pillar avoidance (steering) ──────────────────────────────────────────────
# Rule 9.19 / page 6: red pillar = keep to the RIGHT of the lane, green pillar
# = keep to the LEFT of the lane. Direction is fixed by colour, not by where
# the pillar happens to sit in the frame. None of the numbers below are rule
# values -- the rules define pillar geometry (13.19: 50x50x100mm) and the
# knock-down tolerance circle (13.15: 85mm diameter) but not an avoidance
# trigger distance or steering gain; these are engineering tuning constants
# and should be re-tuned once the real chassis/servo throw is known.
PILLAR_AVOID_TRIGGER_MM = 400.0   # start steering away once closer than this
PILLAR_STEER_MIN         = 0.30   # steer magnitude the instant it triggers
PILLAR_STEER_MAX         = 0.85   # steer magnitude at/near contact distance
...
class CVPipeline:
    ...
    def process(self, frame):
        ...
        pillar_near  = self._is_pillar_near(red_pillar, green_pillar,
                                             threshold_mm=PILLAR_AVOID_TRIGGER_MM)
    ...
    def get_pillar_action(self, red_pos, green_pos, recalled):
        info = self.get_pillar_action_info(red_pos, green_pos, recalled)
        return info["action"] if info else 0.0

    def get_pillar_action_info(self, red_pos, green_pos, recalled):
        """
        Same steering-avoidance decision as get_pillar_action(), but returns
        the full breakdown for display: which colour triggered it, at what
        distance, and which way it's steering.

        Rule 9.19 / rules page 6: red traffic sign -> keep to the RIGHT side
        of the lane, green -> keep to the LEFT side of the lane. Direction is
        therefore fixed by colour alone (red always steers RIGHT/positive,
        green always steers LEFT/negative) -- it must NOT depend on the
        pillar's on-screen x-position, which can sit on either side of the
        frame depending on approach angle and would otherwise risk steering
        the vehicle around the wrong side of the sign.

        Magnitude ramps with real-world distance_mm (from CVPipeline's
        height-based distance estimate, Rule 13.19 pillar geometry), 0 at
        PILLAR_AVOID_TRIGGER_MM out to PILLAR_STEER_MAX at contact distance.
        This is the "how hard to turn from what distance" behaviour.

        Returns:
            None if no pillar is currently known, or it's still farther than
            PILLAR_AVOID_TRIGGER_MM away (nothing to display/act on), else
            {"color", "distance_mm", "action", "direction"}.
        """
        red   = red_pos   or (recalled.get("red")   if recalled else None)
        green = green_pos or (recalled.get("green") if recalled else None)

        red_dist   = red[3]   if red   and len(red)   > 3 else None
        green_dist = green[3] if green and len(green) > 3 else None

        if red_dist is None and green_dist is None:
            return None

        # If both are known, the nearer one is the immediate hazard and
        # takes priority. A None distance is treated as "unknown / not
        # close" so it never wins over a real measurement.
        if red_dist is not None and green_dist is not None:
            color, dist = ("red", red_dist) if red_dist <= green_dist else ("green", green_dist)
        elif red_dist is not None:
            color, dist = "red", red_dist
        else:
            color, dist = "green", green_dist

        if dist >= PILLAR_AVOID_TRIGGER_MM:
            return None

        proximity = (PILLAR_AVOID_TRIGGER_MM - dist) / PILLAR_AVOID_TRIGGER_MM
        proximity = max(0.0, min(1.0, proximity))
        magnitude = PILLAR_STEER_MIN + (PILLAR_STEER_MAX - PILLAR_STEER_MIN) * proximity

        if color == "red":
            action, direction = magnitude, "RIGHT"
        else:
            action, direction = -magnitude, "LEFT"

        return {"color": color, "distance_mm": dist, "action": action, "direction": direction}
