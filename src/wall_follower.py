...
class WallFollower:
    ...
    def lateral_offset_mm(self, x_px, walls):
        """
        Converts a pixel x-coordinate (e.g. a pillar's cx) into a real-world
        lateral distance from the LEFT wall, in mm. Used to store/recall
        "distance from wall to obstacle" instead of just a raw pixel
        position, so recall can steer to a target wall-gap and hold a
        straight line past a known obstacle instead of just reacting to
        the pillar's on-screen position.
        Returns None if walls are missing/invalid.
        """
        if not walls:
            return None
        left  = walls.get("left",  0)
        ...
        corridor_mm = 1000.0 if self.mode == "obstacle" else (
            600.0 if self.corridor_type == "narrow" else 1000.0)
        mm_per_px = corridor_mm / corridor_px
        return (x_px - left) * mm_per_px
